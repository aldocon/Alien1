...
let LI (rd,imm) = ORI (rd,"0",imm) (* LI($rd,imm):   $rd = imm *)
...
and ppMips inst =
  match inst with
    ...
    | SUBI (rd,rs,v) -> "\tsubi\t" + ppReg rd + ", " + ppReg rs + ", " + v
...
and numerical (s: string) =
  Seq.fold (fun i c -> i && (int c) >= (int '0') && (int c) <= (int '9')) true s
